...
class Sidebar {
	...
	updateNodeForm(name) {
		return new Promise((resolve, reject) => {
			...
			form.find("select[name='type']").on('change', e => {
				form.find('#tagsAccordion').replaceWith(this.createTags(data.tags, form.find('select[name="type"]').val()))
			})


			form.find('.removeTag').on("click", (e) => {
				const inputName = $(e.target).parent().parent().find("input").prop("name")
	...
	createTags(tags, type) {
		...
		for (let [key, value] of Object.entries(tags)) {
			if (config.nodes.nodesAvailable[type].tags.findIndex(x => x == key) == -1) {
				if (config.nodes.allowCustomTags) {

...
exports.createContentPage = createContentPage;
exports.discardChanges = discardChanges;
exports.createCollapseEle = createCollapseEle;
exports.roundLatLng = roundLatLng;
exports.arrayToPolygonWkt = arrayToPolygonWkt;
exports.arrayToPolylineWkt = arrayToPolylineWkt;
exports.createInput = createInput;
exports.createSelect = createSelect;
exports.readForm = readForm;
exports.sendEvent = sendEvent;
exports.getArrayRowEntrys = getArrayRowEntrys;
...
function createContentPage(heading, content) {
  $(_globals.config.dom.sidebar).hide();
  $(_globals.config.dom.content.heading).text(heading);
  $(_globals.config.dom.content.body).html(content);

  $(".contentPage").css("visibility", "hidden");
  $(_globals.config.dom.content.container).css("visibility", "visible");
}

function discardChanges(abort) {
  if (_globals.globals.unsavedChanges) {
...
function createCollapseEle(mainId, bodyId) {
  ...
  var div = $("<div class=\"form-group panel-group\"  id=\"" + mainId + "\" role=\"tablist\"></div>");
  var headerBar = $('<div class="panel panel-default"><div class="panel-heading" role="tab" id="tagsPanel"></div></div>');

  headerBar.find("#tagsPanel").append("<h4 class=\"panel-title\"><a role=\"button\" data-toggle=\"collapse\" data-parent=\"#" + mainId + "\" href=\"#" + bodyId + "\" aria-expanded=\"false\" aria-controls=\"collapseOne\">" + heading + "</a></h4>");

  var tabBody = $("<div id=\"" + bodyId + "\" class=\"panel-collapse collapse\" role=\"tabpanel\" aria-expanded=\"false\" aria-labelledby=\"tagsPanel\"><div class=\"panel-body\">" + body + "</div></div>");

  div.append(headerBar).append(tabBody);
  return div;
}

function roundLatLng() {
...
function shortString(string) {
  ...
  if (string) return string.length > length ? string.substring(0, length - 3) + "..." : string;else return "undefined";
}
...
var Store = function () {
   ...
   */
		...
		value: function initData(json) {
			...
			this._json.children.forEach(function (child) {
				// Node Type unknown
				if (typeof _globals.config.nodes.nodesAvailable[child.type] == "undefined") {
					console.log(child.name + " has been removed. Type is unknown.");
					removeItems.push(child);
				} else {
					// Node Type is not enabled
					if (_globals.config.nodes.nodesEnabled.indexOf(child.type) == -1) {
					...
					// Node Type is enabled
							...
							if (typeof child.tags == "undefined") {
								child.tags = {};
							} else if (!_globals.config.nodes.allowCustomTags) {
								...
								try {
									var _loop = function _loop() {
										...
										if (_globals.config.nodes.nodesAvailable[child.type].tags.findIndex(function (x) {
											return x == tag;
										}) == -1) {
							...
							_globals.config.nodes.nodesAvailable[child.type].tags.forEach(function (tag) {
  	...
  	switch (detail.task) {
		...
		value: function addNode(data) {
			...
			return new Promise(function (resolve, reject) {
				...
				if (index !== -1) {
					(0, _modal.modal)("Error", "Element name already exists");
					reject();
					return false;
				}
				...
				for (var i = 0; i < _globals.config.nodes.nodesAvailable[data.type].tags.length; i++) {
					formdata.tags[_globals.config.nodes.nodesAvailable[data.type].tags[i]] = "";
				}
   ...
   */
		...
		key: 'addEdge',
		value: function addEdge(predecessor, successor) {
			var _this5 = this;
			...
			return new Promise(function (resolve, reject) {
				...
				var index = _this5._json.children.findIndex(function (x) {
					return x.name == predecessor;
				});
				if (index !== -1) {
					if (_this5._json.children[index].successors.indexOf(successor) === -1) {
						_this5._json.children[index].successors.push(successor);
					} else {
						reject("Already exists");
					}
				}
				// Add Predecessor
				index = _this5._json.children.findIndex(function (x) {
					return x.name == successor;
				});
				if (index !== -1) {
					if (_this5._json.children[index].predecessors.indexOf(predecessor) === -1) _this5._json.children[index].predecessors.push(predecessor);
				}
		...
		value: function deleteNode(name) {
			...
			return new Promise(function (resolve, reject) {
				_this6._json.children = _this6._json.children.filter(function (child) {
					return child.name != name;
				});

				_this6.deleteRelationNames(name);
				resolve(name);
			});
		}
	}, {
    	...
    	data: updateData.currentId
     	...
     	data: {
			...
			updateData.successors.forEach(function (child) {
				var index = _this7._json.children.findIndex(function (x) {
					return x.name == child;
				});
				if (index !== -1) {
					if (_this7._json.children[index].predecessors.indexOf(updateData.name) === -1) {
						_this7._json.children[index].predecessors.push(updateData.name);
      	...
      	sendEvent("dataChanged", {
			...
			updateData.predecessors.forEach(function (child) {
				...
				if (index !== -1) {
					if (_this7._json.children[index].successors.indexOf(updateData.name) === -1) {
						_this7._json.children[index].successors.push(updateData.name);
		...
		value: function updateNodesEnabled(nodesEnabled) {
			...
			if (this._json != null) {
				...
				diffRemovedTypes.forEach(function (name) {
					...
					console.log(_this8._json.children.filter(function (x) {
						return x.type == name;
					}));

					_this8._json.children.filter(function (x) {
						return x.type == name;
					}).forEach(function (child) {
						_this8.filterElements.push(child);
				...
				this.filterElements.forEach(function (child) {
					...
					if (index != -1) {
						_this8._json.children.splice(index, 1);
					}
				});

				var elesToAdd = [];
				diffAddedTypes.forEach(function (name) {
					elesToAdd = elesToAdd.concat(_this8.filterElements.filter(function (x) {
						return x.type == name;
					}));
				});

				elesToAdd.forEach(function (child) {
					var index = _this8.filterElements.findIndex(function (x) {
						return x.name == child.name;
					});
					_this8._json.children.push(child);

					if (index != -1) {
						_this8.filterElements.splice(index, 1);
					}
				});
...
function modal() {
  ...
  // Get the modal
  var modal = $(_globals.config.dom.modal.container);
  var backdrop = $(_globals.config.dom.modal.backdrop);
  $(_globals.config.dom.modal.heading).text(head);
  $(_globals.config.dom.modal.body).html(content);
...
function hideModal() {
  $(_globals.config.dom.modal.backdrop).removeClass("in");
  $(_globals.config.dom.modal.container).removeClass("in");
}
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var Sidebar = function () {
	...
	_createClass(Sidebar, [{
		...
		value: function updateNodeForm(name) {
			...
			var nodes = _store.store.getAllElements();
			...
			form.append((0, _helper.createSelect)("predecessors", data.predecessors, nodes.filter(function (node) {
				return node != data.name;
			}), "multiple=\"multiple\"")).append((0, _helper.createSelect)("successors", data.successors, nodes.filter(function (node) {
				return node != data.name;
			}), "multiple=\"multiple\""));
...
var _nodeSettings = __webpack_require__(15);

var _selectJson = __webpack_require__(16);

var _globals = __webpack_require__(2);

var _helper = __webpack_require__(1);

var _cytoscape = __webpack_require__(17);
...
function initListenerDataRevieved() {
	...
	$(_globals.config.dom.links.graph).off("click").on("click", function () {
		if ((0, _helper.discardChanges)()) {
			$(".contentPage").css("visibility", "hidden");

			(0, _helper.setActiveMenuItem)(_globals.config.dom.links.graph);
			$(_globals.config.dom.canvasContainer).css("visibility", "visible");
			$(_globals.config.dom.sidebar).show();

			_cytoscape.cyto.show();
		}
	}).click();

	$(_globals.config.dom.links.download).off("click").on("click", function () {
		var bool = confirm("Attach Explorer Positions to json?");
		var data = _store.store.json;
		data.children = _store.store.getAllElements();
		var urlString = "text/json;charset=utf-8,";

		if (bool) {
			for (var i = 0; i < data.children.length; i++) {
				if (typeof data.children[i].pos == "undefined") data.children[i].pos = {};
				Object.assign(data.children[i].pos, _this.cy.$("#" + data.children[i].name).position());
			}
		} else {
...
exports.openJsonSelection = openJsonSelection;
exports.initDropEvents = initDropEvents;
...
function openJsonSelection() {
  ...
  (0, _modal.modal)("Select File", content);
  ...
  $("#useDefault").on('click', function () {
    $.getJSON("minimal-example.json", function (jsonData) {
      ...
      (0, _helper.sendEvent)("dataReceived", jsonData);
      (0, _modal.hideModal)();
    }).fail(function (er) {
      (0, _modal.modal)("Error", "Cross orgin error. Choose json file above or drop it.");
    });
  });
...
function sendFile(file) {
  ...
  reader.onload = function () {
    ...
    try {
      ...
      (0, _helper.sendEvent)("dataReceived", json);
      (0, _modal.hideModal)();
    } catch (e) {
      $('.loaderDiv').hide();
      (0, _modal.modal)("Error", "File not supported");
      console.log("Error", e);
    }
  };
...
var CytoScape = function () {
	function CytoScape() {
		...
		this.cy = cytoscape({
			container: $(_globals.config.dom.canvasContainer),
			layout: {},
			style: _globals.config.cytoscape.styleConfig
		});

		this.initMarkers();
		this.initEvents();
		//this.registerEvents()
		this.autoLayout = localStorage.getItem("autoLayout") ? localStorage.getItem("autoLayout") : _globals.globals.autoLayout;
	}

	_createClass(CytoScape, [{
		key: 'show',
		value: function show() {
			this.initElements(_store.store.getElements());
		}
	}, {
		key: 'initMarkers',
		value: function initMarkers() {
			...
			Object.keys(_globals.config.nodes.nodesAvailable).forEach(function (type) {
				_this.cy.style().selector('node[type="' + type + '"]').style({
					'background-color': _globals.config.nodes.nodesAvailable[type].color,
					'background-image': _globals.config.nodes.nodesAvailable[type].icon,
  	...
  	document.addEventListener("dataChanged", (e) => {
			...
			this.cy.contextMenus({
				menuItems: [{
					...
					onClickFunction: function onClickFunction(event) {
						if (!(0, _helper.discardChanges)()) return;

						_store.store.deleteNode(event.cyTarget.id()).then(function (name) {
							_this2.deleteNode(name);
						});
					}
				}, {
					id: 'add-successors',
					...
					onClickFunction: function onClickFunction(event) {
						...
						_globals.globals.unsavedChanges = true;
						...
						_this2.cy.on("mouseover", "node", {}, function (_event) {
							var evtToTarget = _event.target || _event.cyTarget;
							_this2.cy.$('#shadowEdge').move({
								target: evtToTarget.data().id
							});
						});

						_this2.cy.on("click", "node", {}, function (_event) {
							var evtToTarget = _event.target || _event.cyTarget;
					...
					onClickFunction: function onClickFunction(event) {
						...
						_store.store.deleteEdge(event.cyTarget.source().id(), event.cyTarget.target().id()).then(function () {
							_this2.deleteEdge(event.cyTarget.source().id(), event.cyTarget.target().id());
						});
					...
					title: 'Add node',
					coreAsWell: true,
					onClickFunction: async function onClickFunction(event) {
						if (!(0, _helper.discardChanges)()) return;
		...
		key: 'initElements',
		value: function initElements(eles) {
			this.cy.remove("*");
			this.addNodes(eles);
		}
	}, {
  ...
  changeType(name, newType) {
  	...
  	this.cy.add(edges)
		...
		value: function updateLayout() {
			...
			this.cy.makeLayout({
				name: localStorage.getItem("style") || _globals.config.cytoscape.defaultStyle
			}).run();
		}
	}]);
...
var cyto = exports.cyto = new CytoScape();
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
